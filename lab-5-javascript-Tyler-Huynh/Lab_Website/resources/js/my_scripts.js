...
function loadStatsPage(){
  var table = document.getElementById("stats_table"); //retrieving the table element 
  var row_counter; //counter for the row 
  var opp_score; //the current opposing team score
  var home_score; //the current home score
  var home_wins = 0; //home wins
  var home_losses = 0; //home losses
  var who_won;
  var row = table.rows[row_counter]; 

  for(row_counter = 2; row_counter < table.rows.length; row_counter++){ //for loop for interating through the table
      home_score = parseInt(table.rows[row_counter].cells[2].innerHTML); //grabs the values from that specific cell
      opp_score = parseInt(table.rows[row_counter].cells[3].innerHTML);

      if (home_score > opp_score){ //seeing if the home score is greater than the opp score
        home_wins +=1; //incrementing
        table.rows[row_counter].cells[4].innerHTML = "CU Buffs";
      }else if(home_score < opp_score){
        home_losses += 1; //incrementing
        who_won = table.rows[row_counter].cells[1].innerHTML;
        table.rows[row_counter].cells[4].innerHTML = who_won;
      }
  }
  document.getElementById("wins").innerHTML = home_wins;
  document.getElementById("losses").innerHTML = home_losses;
}
...
function loadPlayersPage(){
  var dropdown = document.getElementById("player_selector"); //retrieving the table element 
  var playerCounter; //player index

  for (playerCounter = 0; playerCounter < players.length; playerCounter++){ //iterating through players array
    var anchor = document.createElement("option");
    anchor.href = "#";
    anchor.setAttribute("onclick", "switchPlayers(" + playerCounter + ")");
    anchor.innerHTML = players[playerCounter].name;
...
function switchPlayers(playerNum){
  var playerStats = players[playerNum];

  //main player info
  document.getElementById("p_year").innerHTML = playerStats.year;
  document.getElementById("p_major").innerHTML = playerStats.major;
  document.getElementById("g_played").innerHTML = playerStats.games_played;

  //player images
  document.getElementById("player_img").src = playerStats.img;
  document.getElementById("player_img").alt = playerStats.name;

  //player stats
  document.getElementById("p_yards").innerHTML = playerStats.pass_yards;
  document.getElementById("r_yards").innerHTML = playerStats.rushing_yards;
  document.getElementById("rec_yards").innerHTML = playerStats.receiving_yards;

  //calculated player stats
  document.getElementById("avg_p_yards").innerHTML = Math.round((parseFloat(playerStats.pass_yards)) / (parseFloat(playerStats.games_played)));
  document.getElementById("avg_r_yards").innerHTML = Math.round((parseFloat(playerStats.rushing_yards)) / (parseFloat(playerStats.games_played)));
  document.getElementById("avg_rec_yards").innerHTML = Math.round((parseFloat(playerStats.receiving_yards)) / (parseFloat(playerStats.games_played)));
}